...
$(function () {
    ...
    var mescroll = new MeScroll("mescroll", {
        up: {
            ...
            clearEmptyId: "myRecord-list", //1.下拉刷新时会自动先清空此列表,再加入数据; 2.无任何数据时会在此列表自动提示空
            empty: {
    ...
    function setListData(curPageData) {
        var listDom = document.getElementById("myRecord-list");
        var result = '';
        for (var i = 0; i < curPageData.length; i++) {
            var pd = curPageData[i];
            result += '<li class="aui-list-item aui-list-item-middle aui-margin-b-15">' +
            '<div class="aui-media-list-item-inner">' +
                '<a class="aui-list-item-media myRecord-list-item-media" href="' + pd.myRecordListHref + '">' +
                '<img src="' + pd.myRecordListImg + '" class="aui-list-img-sm myRecord-list-img">' +
                '</a>' +
                '<div class="aui-list-item-inner">' +
                '<div class="aui-list-item-text myRecord-list-text">' +
                '<div class="aui-list-item-title myRecord-song-name">' + pd.myRecordSongName + '</div>' +
                '<div class="aui-list-item-right myRecord-share">' +
                '<i class="aui-iconfont aui-icon-close js-delete-this" tapmode onclick="deleteThis(this);">' +
                '</i>' +
                '</div>' +
                '</div>' +
                ...
                '<span class="per-listen-num">' + pd.perListenNum + '</span>' +
                '</div>' +
                '<div class="myRecord-text-icons">' +
                '<i class="iconfont icon-dianzan">' + '</i>' +
                '<span class="per-like-num">' + pd.perLikeNum + '</span>' +
                '</div>' +
                '</div>' +
                '<div class="aui-list-item-right per-myRecord-time">' + pd.perMyRecordTime + '</div>' +
                '</div>' +
                '</div>' +
                '</div>' +
                '</li>';
