...
$(function () {
    ...
    sessionStorage.setItem("resultStorage", objStorage);//返回{"deviceId": state,"openId": 123}
        ...
        if ('WebSocket' in window) {
            websocket = new WebSocket("ws://192.168.1.141:8082/webSocketServer?deviceId=123456&openId=oVdmm1f0FIMOtVMwS-tkBKWf79Rx&type=wechat");
        }
        else {
        ...
        websocket.onmessage = function(msg) {
            ...
            $('#control-btn').click(function () {
                if(deal && deal!='' && url.indexOf("deviceId")!=-1){  //只有在已经登录并且购买了套餐的情况下才能点击进去；其他情况都要弹窗提醒登录或者购买套餐
            ...
            $('#singer-btn').click(function () {
                if(deal && deal!='' && url.indexOf("deviceId")!=-1){
        ...
        */
        $('#buy-package').click(function () {
            if(url.indexOf("deviceId")!=-1){
                window.location.href='http://wechat.uniquemusic.cn/packagesPage?deviceId='+state;
            }else{
                dialog.alert({
                    title:"是否扫码畅享点歌体验？",
                    buttons:['取消','确定']
